...
class Window:
	...
	def __init__(self, root):
		...
		Button(self.root, text = "Kaydet", command = self.savefile).grid(row=0, column = 1)
		Button(self.root, text = "Ac", command = self.openfile).grid(row=0, column = 2)
	...
	def savefile(self):
		...
		Label(savegui, text = "Dosya Adi").grid(row=0, column = 0)
